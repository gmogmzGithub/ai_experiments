...
LeyesBiblio_url = os.environ["LEYES_NUEVO_LEON"]
...
logger.debug("Comenzando con las Leyes de Nuevo León")
response = requests.get(LeyesBiblio_url)
soup = BeautifulSoup(response.text, 'html.parser')

# Look for all <a> tags with href starting with "pdf/"
pdf_links = soup.find_all('a', href=lambda href: href and "hcnl.gob.mx/trabajo_legislativo/leyes/pdf" in href)
pdf_hrefs = [link['href'] for link in pdf_links]

# Filter out URLs ending with .doc or .docx
pdf_hrefs = [pdf for pdf in pdf_hrefs if not (pdf.endswith('.doc') or pdf.endswith('.docx'))]

for pdf_url in tqdm(pdf_hrefs, desc="Descargando los PDFs de Nuevo León 🦁", unit="file"):
    ...
    folder_path = os.path.join(datasets, "nuevo_leon")
